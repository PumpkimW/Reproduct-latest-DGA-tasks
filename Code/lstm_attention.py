#这个代码是复现2022年 CCF B的论文On Detecting and Classifying DGA Botnets and their Families
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.lstm1=nn.LSTM(input_size=embedding_dim,hidden_size=18,num_layers=1,batch_first=True,bidirectional=False)  #batch_first表示是否将batch放在第一个,hidden_size表示多少个神经元
        self.attention1=nn.MultiheadAttention(embed_dim=18,num_heads=1)
        self.dropout1=nn.Dropout(p=0.5)
        self.lstm2=nn.LSTM(input_size=18,hidden_size=5,num_layers=1,batch_first=True,bidirectional=False)
        self.attention=nn.MultiheadAttention(embed_dim=5,num_heads=1)
        self.flatten=nn.Flatten()
        self.linear=nn.Linear(max_document_length*5,classfication)
        ...
        self.linear4=nn.Linear(5,5)
        self.linear5=nn.Linear(5,5)
        self.linear6=nn.Linear(5,5)
    ...
    def forward(self,x):
        ...
        # attn_output, attn_output_weights = self.attention1(output,tmp1,tmp2)   #(query,key,value)


        x=self.dropout1(output)
        output,(h_n,c_n)=self.lstm2(x)   #维度是(batch_size,seq_length,hidden_size*2) 这里的hidden_size进行了改变

        tmp1=self.linear4(output)
        tmp2=self.linear5(output)
        # output=self.linear6(output)

        # attn_output, attn_output_weights = self.attention(output,output,output)   #(query,key,value)
