...
class Model(nn.Module):
    def __init__(self):
        ...
        self.Cnn_model2=nn.Sequential(
            nn.Conv1d(in_channels=48,out_channels=kernel_number,kernel_size=2,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        self.Cnn_model3=nn.Sequential(
            nn.Conv1d(in_channels=48,out_channels=kernel_number,kernel_size=3,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        self.Cnn_model4=nn.Sequential(
            nn.Conv1d(in_channels=48,out_channels=kernel_number,kernel_size=4,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        self.Cnn_model5=nn.Sequential(
            nn.Conv1d(in_channels=48,out_channels=kernel_number,kernel_size=5,stride=1),#我这里还是第二维是域名的长度，第三维是特征，out_channel理解为卷积核的个
            nn.MaxPool1d(kernel_size=2)  #这个地方就是第三个维度直接用输入除以输出
        )
        ...
        self.linear=nn.Linear(max_document_length*310,classfication)  
        self.softmax=nn.Softmax(dim=1)
    def forward(self,x):
        x=self.embedding1(x)      #维度是(batch_size,seq_length,embedding_dim)

        x1=self.Cnn_model2(x)
        ...
        tmp3=self.linear3(output)
        attn_output, attn_output_weights = self.attention(tmp1,tmp2,tmp3)   #(query,key,value)

        x0=torch.cat((x1,x2,x3,x4,attn_output),dim=2)   #这个地方是最终的输入维度（batch_size,max_document_length,310）

        output=self.flatten(x0)    #维度是(batch_size,max_document_length*hidden_size)
